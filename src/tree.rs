...
impl<T: Value> Tree<T> {
    ...
    pub fn with_updated_leaves<U: UpdateMap<T>>(
        ...
        match self {
            ...
            Self::PackedLeaf(pl) if depth == 0 => Ok(Arc::new(Self::PackedLeaf(pl.update(
                prefix + offset,
                hash,
                updates,
            )?))),
            Self::Node { left, right, .. } if depth > 0 => {
                ...
                let has_left_updates =
                    updates.has_any_in_range(left_prefix + offset, right_prefix + offset);
                let has_right_updates =
                    updates.has_any_in_range(right_prefix + offset, right_subtree_end + offset);
            ...
            Self::Zero(zero_depth) if *zero_depth == depth => {
                if depth == 0 {
                    if opt_packing_factor::<T>().is_some() {
                        let pl = PackedLeaf::empty().update(prefix + offset, hash, updates)?;
