...
theorem ProgressiveList.get_mut_present_succeeds_of_fallback {T U : Type}
  ...
  obtain ⟨value, hread, hinitial⟩ : ∃ value,
      ...
      ((mapInst.get self.updates index = ok (some old) ∧ value = old) ∨
       (mapInst.get self.updates index = ok none ∧
        ValueInst.corecloneCloneInst.clone old = ok value)) := by
    have hread := ProgressiveList.get_mut_read_eq_pending_or_clone_of_fallback
...
theorem ProgressiveList.get_mut_spec_of_fallback {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T)
  ...
  have hget : ProgressiveList.get ValueInst mapInst self index = ok (some contents[index.val]) := by
    rw [hrep.2 index]
    simp [hindex]
  obtain ⟨value, back, hmut, hinitial⟩ := ProgressiveList.get_mut_present_succeeds_of_fallback
    ValueInst mapInst self index contents[index.val] hreads hget hclone
  refine ⟨value, back, hmut, hinitial, ?_⟩
  intro replacement
  refine ⟨ProgressiveList.get_mut_represents_set_of_fallback ValueInst mapInst self contents index replacement
...
theorem ProgressiveList.get_mut_total_spec_of_fallback {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T)
    (self : ProgressiveList T U) (contents : _root_.List T) (index : Std.Usize)
    (hrep : self.Represents ValueInst mapInst contents)
    (hreads : self.GetMutReads ValueInst mapInst index)
    (hwrites : index.val < contents.length → self.GetMutWriteReads ValueInst mapInst index)
    (hmax : index.val < contents.length →
      self.GetMutMaxIndexAgrees ValueInst mapInst index)
    (hmissing : contents.length ≤ index.val → self.GetMutMissing ValueInst mapInst index)
  ...
  · have hsome : contents[index.val]? = some contents[index.val] := by simp [hindex]
    obtain ⟨value, back, hmut, hinitial, hwrite⟩ := ProgressiveList.get_mut_spec_of_fallback
  ...
  · have hout : contents.length ≤ index.val := by omega
    obtain ⟨back, hmut, hback⟩ := ProgressiveList.get_mut_out_of_bounds_of_fallback
