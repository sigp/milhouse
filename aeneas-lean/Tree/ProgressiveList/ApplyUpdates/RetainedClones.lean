...
forces identity of every selected pending clone and retained stored clone.
Input representation and backing validity supply the original overlay. No
range, maximum, default, clone, or termination law is assumed; the output
needs density but no separate capacity-validity assumption. -/
theorem ProgressiveList.apply_updates_nonempty_retained_clone_identity {T U : Type}
  ...
  · rw [hempty] at htrue
    cases htrue
  · apply progressive_tree.ProgressiveTree.with_updated_leaves_retained_clone_identity
      ValueInst mapInst self.updates hlayout hbacking.1.shape hupdate
