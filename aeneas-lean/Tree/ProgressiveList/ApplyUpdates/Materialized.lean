...
suffixes. No separate false-answer extent or default-map law is required. -/
theorem ProgressiveList.apply_updates_nonempty_backing_contents_of_layer_selection {T U : Type}
...
theorem ProgressiveList.apply_updates_nonempty_backing_contents_of_layer_agreement {T U : Type}
    ...
    (happly : ProgressiveList.apply_updates ValueInst mapInst self = ok (.Ok (), result)) :
    result.tree.elements = contents := by
  exact ProgressiveList.apply_updates_nonempty_backing_contents_of_layer_selection
    ValueInst mapInst self contents hlayout hclone
    (fun maximum hmax start stop hquery => (hextents maximum hmax start stop hquery).selectsInside)
    hrange hlayers hskipped hrep hbacking hempty happly
