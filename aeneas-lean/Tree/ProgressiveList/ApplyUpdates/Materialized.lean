...
theorem ProgressiveList.apply_updates_nonempty_backing_contents_of_skipped {T U : Type}
  ...
  obtain ⟨_, _, hreads⟩ := ProgressiveList.apply_updates_nonempty_backing_spec_of_skipped
    ValueInst mapInst self contents hlayout hclone (fun maximum hmax => (hrange maximum hmax).excludesValues)
    hskipped hrep hbacking.1.shape (by simpa using hbacking.1.endsAfter) hempty happly
