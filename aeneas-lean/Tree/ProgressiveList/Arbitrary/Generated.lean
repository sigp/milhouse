...
theorem ProgressiveList.arbitrary_success_spec {T U : Type}
  ...
  obtain ⟨hrep, hspine, hpending⟩ :=
    ProgressiveList.try_from_vec_spec ValueInst mapInst values hlayout hdefault hnew
  exact ⟨htrace, hlength, (by simpa only [helements] using hrep),
    ProgressiveList.new_backing_valid ValueInst mapInst hlayout values hnew, hspine, hpending⟩
