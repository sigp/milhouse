...
/-- Given actual packing query results, the selected arithmetic, binary geometry, and
...
theorem ProgressiveList.rebase_on_success_iff_requirements {T U : Type}
    ...
    (hqueries : progressive_tree.RebasePackingQueries ValueInst.tree_hashTreeHashInst factor packingDepth)
  ...
  · rintro ⟨result, hrebase⟩
    ...
    exact (progressive_tree.ProgressiveTree.rebase_on_success_iff_requirements ValueInst hqueries
      self.tree base.tree self.length base.length).mp ⟨newTree, htree⟩
  · intro hinputs
    obtain ⟨newTree, htree⟩ := (progressive_tree.ProgressiveTree.rebase_on_success_iff_requirements ValueInst hqueries
...
Only the actual packing query results are assumed outside this criterion;
no packing-coherence law is needed. -/
theorem ProgressiveList.rebase_success_iff_requirements {T U : Type}
  ...
  · rintro ⟨result, hrebase⟩
    ...
    exact ⟨⟨updates, hupdates⟩, (ProgressiveList.rebase_on_success_iff_requirements ValueInst mapInst hqueries
      { self with updates } base).mp ⟨result, hrebased⟩⟩
  · rintro ⟨⟨updates, hclone⟩, hinputs⟩
    obtain ⟨result, hrebase⟩ := (ProgressiveList.rebase_on_success_iff_requirements ValueInst mapInst hqueries
