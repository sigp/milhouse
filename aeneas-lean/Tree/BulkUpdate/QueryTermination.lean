...
/-- Successful binary rebuilding certifies termination of every scoped range
query. Layout and prefix alignment identify the actual child windows. No input
shape, density, capacity, clone, lookup, or range-correctness law is assumed. -/
theorem Tree.with_updated_leaves_queries_terminate {T U : Type}
  ...
  have aux : ∀ (treeDepth : Nat) (depth : Std.Usize) (before after : Tree T) (prefix1 : Std.Usize),
    ...
    | zero =>
      intro _ _ _ _ _ _ _
      exact BulkRangeOn.zero _ _ _ _ _
    | succ child ih =>
      intro depth before after prefix1 hdepth halign hupdate
      have node_queries : ∀ hash left right,
        ...
        cases hquery with
        | left_here hqueryLo hqueryHi =>
          have heqLo : queryLo = lo := by scalar_tac
          have heqHi : queryHi = middle := by scalar_tac
          subst queryLo
          subst queryHi
          rcases leftStep with ⟨hfalse, _⟩ | ⟨htrue, _⟩
          · exact ⟨false, hfalse⟩
          · exact ⟨true, htrue⟩
        | right_here hqueryLo hqueryHi =>
          have heqLo : queryLo = middle := by scalar_tac
          have heqHi : queryHi = stop := by scalar_tac
          subst queryLo
          subst queryHi
          rcases rightStep with ⟨hfalse, _⟩ | ⟨htrue, _⟩
          · exact ⟨false, hfalse⟩
          · exact ⟨true, htrue⟩
        | @left_tail _ _ selectedLo selectedHi _ _ hqueryLo hqueryHi hselected htail =>
          ...
          · rw [hselected] at hempty
            cases hempty
          · exact ih nd left newLeft prefix1 hndDepth halignLeft hleft _ _ htail
        | @right_tail _ _ selectedLo selectedHi _ _ hqueryLo hqueryHi hselected htail =>
          ...
          · rw [hselected] at hempty
            cases hempty
          · have hqueries : BulkRangeOn
                (fun lo hi => ∃ answer, mapInst.has_any_in_range updates lo hi = ok answer)
                mapInst updates factor child
                (prefix1.val + offset.val + subtreeCapacity factor child) := by
            ...
            exact hqueries _ _ htail
      cases before with
      | Node hash left right => exact node_queries hash left right hupdate
      | Leaf leaf =>
      ...
      | Zero oldDepth =>
        ...
        · have hz : depth ≠ 0#usize := by scalar_tac
          ...
          exact node_queries _ _ _ hupdate
