...
/-- Updating a nonempty progressive layer preserves binary shape and applies
    ...
    Skipped pending values need only agree with the original binary slots. -/
theorem ProgressiveTree.updated_layer_contents_of_skipped {T U : Type}
    ...
    (hagreement : before.BulkSkippedValuesAgree mapInst updates factor binary.val 0 start.val)
  ...
  obtain ⟨hfit, hafter, hcontents⟩ := tree.Tree.with_updated_leaves_capacity_shape_contents_of_skipped
    ValueInst mapInst updates hlayout
    (by simpa only [show (0#usize).val = 0 from rfl, Nat.zero_add] using hclone)
    (by simpa only [show (0#usize).val = 0 from rfl, Nat.zero_add] using hagreement) hshape' (by simp) hoffset hupdate
...
theorem ProgressiveTree.get_after_updated_layer_of_skipped {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
  ...
  obtain ⟨_, hoffset, hwidth, _⟩ := ProgressiveTree.updated_layer_contents_of_skipped
    ValueInst mapInst updates hlayout hclone hagreement hnext hstart hstop hbinary
    (by omega) hshape hupdate
  have hbinaryVal := ProgressiveTree.binary_depth_successor_val ValueInst hnext hbinary
  have hshape' : before.Shape factor binary.val := by simpa only [hbinaryVal] using hshape
  have hlocal := saturating_sub_val query start
  have hread := tree.Tree.get_after_with_updated_leaves_of_skipped ValueInst mapInst updates
    hlayout (by simpa only [show (0#usize).val = 0 from rfl, Nat.zero_add] using hclone)
    (by simpa only [show (0#usize).val = 0 from rfl, Nat.zero_add] using hagreement) hshape' (by simp) hoffset
...
theorem ProgressiveTree.get_after_expanded_layer_of_skipped {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    {depth next : Std.U32} {start stop binary query : Std.Usize}
    ...
    {hash : lock_api.rwlock.RwLock parking_lot.raw_rwlock.RawRwLock
      (alloy_primitives.bits.fixed.FixedBytes 32#usize)}
    {pending : Option T}
    (hclone : (tree.Tree.Zero binary : tree.Tree T).BulkRetainedCloneOn
      (fun value => ValueInst.corecloneCloneInst.clone value = ok value)
      mapInst updates factor binary.val start.val)
    (hagreement : (tree.Tree.Zero binary : tree.Tree T).BulkSkippedValuesAgree mapInst updates factor binary.val 0 start.val)
    (hnext : depth + 1#u32 = ok next)
    (hstart : ProgressiveTree.total_capacity_at_depth ValueInst depth = ok start)
    (hstop : ProgressiveTree.total_capacity_at_depth ValueInst next = ok stop)
    (hbinary : ProgressiveTree.prog_depth_to_binary_depth ValueInst next = ok binary)
  ...
  have hread := ProgressiveTree.get_after_updated_layer_of_skipped ValueInst mapInst updates
    hlayout hclone hagreement (oldHash := hash) (oldRight := .ProgressiveZero)
...
theorem ProgressiveTree.get_after_updated_layer_override_of_skipped {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    {depth next : Std.U32} {start stop binary query : Std.Usize}
  ...
  obtain ⟨_, _, hwidth, _⟩ := ProgressiveTree.updated_layer_contents_of_skipped ValueInst mapInst updates
    hlayout hclone hagreement hnext hstart hstop hbinary (by omega) hshape hupdate
  have hbinaryVal := ProgressiveTree.binary_depth_successor_val ValueInst hnext hbinary
  have hshape' : before.Shape factor binary.val := by simpa only [hbinaryVal] using hshape
  ...
  have hread := ProgressiveTree.get_after_updated_layer_of_skipped ValueInst mapInst updates hlayout hclone hagreement
...
/-- The existing exclusion contract follows from skipped-value agreement. -/
theorem ProgressiveTree.updated_layer_contents {T U : Type}
    ...
    (hclone : before.BulkRetainedCloneOn (fun value => ValueInst.corecloneCloneInst.clone value = ok value)
      mapInst updates factor binary.val start.val)
    (hrange : tree.BulkRangeOn (update_map.RangeExcludesValuesAt mapInst updates)
      mapInst updates factor binary.val start.val)
    (hnext : depth + 1#u32 = ok next)
    (hstart : ProgressiveTree.total_capacity_at_depth ValueInst depth = ok start)
    (hstop : ProgressiveTree.total_capacity_at_depth ValueInst next = ok stop)
    ...
    after.Shape factor (2 * depth.val) ∧
      ...
      stop.val = start.val + tree.subtreeCapacity factor binary.val ∧
      tree.Tree.BulkContents mapInst updates factor before after binary.val 0 start.val := by
  exact ProgressiveTree.updated_layer_contents_of_skipped ValueInst mapInst updates hlayout hclone
    (tree.Tree.BulkSkippedValuesAgree.of_ranges before binary.val 0 start.val
      (by simpa only [Nat.zero_add] using hrange))
    hnext hstart hstop hbinary hnonempty hshape hupdate
...
theorem ProgressiveTree.get_after_updated_layer {T U : Type}
    ...
    ProgressiveTree.get_recursive ValueInst (.ProgressiveNode newHash after newRight) query depth =
      (do let previous ← ProgressiveTree.get_recursive ValueInst
            (.ProgressiveNode oldHash before oldRight) query depth
          ok (pending.or previous)) := by
  exact ProgressiveTree.get_after_updated_layer_of_skipped ValueInst mapInst updates hlayout hclone
    (tree.Tree.BulkSkippedValuesAgree.of_ranges before binary.val 0 start.val
      (by simpa only [Nat.zero_add] using hrange))
    hnext hstart hstop hbinary hshape hqueryLo hqueryHi hget hupdate
...
theorem ProgressiveTree.get_after_expanded_layer {T U : Type}
    ...
    ProgressiveTree.get_recursive ValueInst (.ProgressiveNode hash after right) query depth =
      ok pending := by
  exact ProgressiveTree.get_after_expanded_layer_of_skipped ValueInst mapInst updates hlayout hclone
    (tree.Tree.BulkSkippedValuesAgree.of_ranges (tree.Tree.Zero binary : tree.Tree T) binary.val 0 start.val
      (by simpa only [Nat.zero_add] using hrange))
    hnext hstart hstop hbinary hqueryLo hqueryHi hget hupdate
...
theorem ProgressiveTree.get_after_updated_layer_override {T U : Type}
    ...
    ProgressiveTree.get_recursive ValueInst (.ProgressiveNode newHash after newRight) query depth =
      ...
      | some value => ok (some value)
      | none => ProgressiveTree.get_recursive ValueInst (.ProgressiveNode oldHash before oldRight) query depth := by
  exact ProgressiveTree.get_after_updated_layer_override_of_skipped ValueInst mapInst updates hlayout hclone
    (tree.Tree.BulkSkippedValuesAgree.of_ranges before binary.val 0 start.val
      (by simpa only [Nat.zero_add] using hrange))
    hnext hstart hstop hbinary hshape hqueryLo hqueryHi hget hupdate
