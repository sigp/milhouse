...
theorem ProgressiveTree.updated_layer_dense {T U : Type}
    ...
    subtreeCapacity factor (2 * depth.val) < 2 ^ System.Platform.numBits ∧
      DenseTree factor after (2 * depth.val)
        (min (newLength - progressiveCapacity factor depth.val) (subtreeCapacity factor (2 * depth.val))) := by
  exact ProgressiveTree.updated_layer_dense_of_extent ValueInst mapInst updates hlayout
    (hlocal.preservesExtent hdomain) hrange hdomain hnext hstart hbinary hhas hdense hupdate
...
private theorem bulk_dense_aux {T U : Type}
  ...
  | h fuel ih =>
    intro depth before after hfuel hdense hfit hlayers hrange hupdate
    ...
    have rightCorrect : ∀ right newRight : ProgressiveTree T,
      ...
      intro right newRight hdense hfit hlayerRanges hranges hright
      ...
      · exact ih _ hless next right newRight (Nat.le_refl _) hdense hfit (hlayerRanges hselected) (hranges hselected) hrecursive
    ...
    | zero hempty =>
      have hold : oldLength ≤ progressiveCapacity factor depth.val := by
        ...
        omega
      have hlength := ProgressiveTree.length_le_of_empty_zero_layer_of_extent ValueInst mapInst hlayout
        (hlayers.here hgeometry) hnext hstart hstop hold hempty
    ...
    | @expand right newLeft hash hhas hleft hright =>
      ...
      obtain ⟨hrightDense, hrightFit⟩ := rightCorrect .ProgressiveZero right hrightDense trivial
        (hlayers.zero_right hgeometry hhas) (hrange.zero_right hgeometry hhas) hright
    ...
    | @node left newLeft right newRight oldHash newHash hleft hright =>
      ...
      obtain ⟨hnewRight, hnewRightFit⟩ := rightCorrect right newRight
        ...
        (hlayers.node_right hgeometry hhasSuccess) (hrange.node_right hgeometry hhasSuccess) hright
      have leftCorrect : DenseTree factor newLeft (2 * depth.val)
        ...
        · obtain ⟨actualStart, actualStop, hactualStart, hactualStop, hstartExact, hwidth, _⟩ :=
          ...
          cases hactualStart
          cases hactualStop
          have heq := ProgressiveTree.has_updates_in_range_false_extent ValueInst mapInst
            (hlayers.here hgeometry) hempty
...
is confined to selected subtrees; the maximum supplies only a numeric bound. -/
theorem ProgressiveTree.with_updated_leaves_recursive_dense_of_range_extents {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : PackingLayout ValueInst factor packingDepth)
    {maximum : Option Std.Usize} {oldLength : Nat} {newLength : Std.Usize}
...
to reflect the presence or absence of pending values. -/
theorem ProgressiveTree.with_updated_leaves_dense_of_range_extents {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : PackingLayout ValueInst factor packingDepth)
...
reflection and the input dense update domain remain sufficient for density. -/
theorem ProgressiveTree.with_updated_leaves_recursive_dense_of_layer_agreement {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : PackingLayout ValueInst factor packingDepth)
    {maximum : Option Std.Usize} {oldLength : Nat} {newLength : Std.Usize}
    ...
    after.Dense factor depth.val (newLength.val - progressiveCapacity factor depth.val) ∧
      after.Fits factor depth.val := by
  exact ProgressiveTree.with_updated_leaves_recursive_dense_of_range_extents ValueInst mapInst updates
    ...
    hrange hdense hfit hupdate

/-- Public density under skipped-layer agreement and positive layer selection.
No independent false-answer length law is required. -/
theorem ProgressiveTree.with_updated_leaves_dense_of_layer_agreement {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : PackingLayout ValueInst factor packingDepth)
    {oldLength : Nat} {newLength : Std.Usize}
  ...
  have hdense' : before.Dense factor (0#u32).val
    ...
    simpa [progressiveCapacity] using hdense
  exact ProgressiveTree.with_updated_leaves_dense_of_range_extents ValueInst mapInst updates
    hlayout hextent hdomain
...
theorem ProgressiveTree.with_updated_leaves_recursive_dense_of_extent {T U : Type}
    ...
    after.Dense factor depth.val (newLength.val - progressiveCapacity factor depth.val) ∧
      after.Fits factor depth.val := by
  exact ProgressiveTree.with_updated_leaves_recursive_dense_of_range_extents ValueInst mapInst updates
    hlayout hextent hdomain (hrange.layers (fun _ _ h => h.preservesExtent hdomain))
    hrange.binary_layers hdense hfit hupdate
...
theorem ProgressiveTree.with_updated_leaves_dense_of_extent {T U : Type}
  ...
  exact ProgressiveTree.with_updated_leaves_dense_of_range_extents ValueInst mapInst updates
    hlayout hextent hdomain
    (fun maximum hmax => (hrange maximum hmax).layers (fun _ _ h => h.preservesExtent hdomain))
    (fun maximum hmax => (hrange maximum hmax).binary_layers) hdense hfit hupdate
