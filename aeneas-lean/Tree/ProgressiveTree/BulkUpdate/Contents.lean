...
private theorem bulk_contents_aux {T U : Type}
  ...
  | h fuel ih =>
    intro depth before after hfuel hshape hends hclone hrange hlayers hskipped hupdate
    ...
    have rightCorrect : ∀ (right newRight : ProgressiveTree T),
        ...
        ((∃ last, maximum = some last ∧ stop.val ≤ last.val) →
          right.BulkBinaryRangeOn (update_map.RangeExcludesValuesAt mapInst updates)
            ValueInst mapInst updates factor maximum next) →
        ((∃ last, maximum = some last ∧ stop.val ≤ last.val) →
      ...
      intro right newRight hrightShape hrightEnds hclones hranges hlayerAgrees hagree hskips hright
      ...
      · exact ih _ hless next right newRight (Nat.le_refl _) hrightShape hrightEnds
          (hclones hselected) (hranges hselected) (hlayerAgrees hselected) (hskips hselected) hrecursive
    ...
    | @expand right newLeft hash hhas hleft hright =>
      ...
      have hleftRanges := hrange.zero_left hgeometry hhas
...
theorem ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_layer_agreement {T U : Type}
  ...
  exact bulk_contents_aux ValueInst mapInst updates hlayout hextent hcomplete
    (Std.U32.max - depth.val) depth before after (Nat.le_refl _) hshape hends hclone hrange hlayers hskipped hupdate
...
to actual external answers and input reads; successful traversal is derived. -/
theorem ProgressiveTree.with_updated_leaves_shape_contents_of_layer_agreement {T U : Type}
  ...
  | ok maximum =>
    ...
    exact ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_layer_agreement ValueInst mapInst updates
