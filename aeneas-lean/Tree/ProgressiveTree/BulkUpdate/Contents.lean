...
private theorem bulk_contents_aux {T U : Type}
    ...
    ∀ (fuel : Nat) (depth : Std.U32) (before after : ProgressiveTree T),
      ...
      before.BulkBinarySkippedValuesAgree
        ValueInst mapInst updates factor maximum depth →
      before.BulkLayerSkippedValuesAgree ValueInst mapInst updates maximum newLength.val depth →
  ...
  | h fuel ih =>
    intro depth before after hfuel hshape hends hclone hbinaryAgreement hlayers hskipped hupdate
    ...
    have rightCorrect : ∀ (right newRight : ProgressiveTree T),
        ...
        ((∃ last, maximum = some last ∧ stop.val ≤ last.val) →
          right.BulkBinarySkippedValuesAgree
            ValueInst mapInst updates factor maximum next) →
        ((∃ last, maximum = some last ∧ stop.val ≤ last.val) →
          right.BulkLayerSkippedValuesAgree ValueInst mapInst updates maximum newLength.val next) →
      ...
      intro right newRight hrightShape hrightEnds hclones hbinaries hlayerAgrees hagree hskips hright
      ...
      · exact ih _ hless next right newRight (Nat.le_refl _) hrightShape hrightEnds
          (hclones hselected) (hbinaries hselected) (hlayerAgrees hselected) (hskips hselected) hrecursive
    ...
    cases hstep with
    | zero hempty =>
      have hlength := ProgressiveTree.length_le_of_empty_zero_layer_of_agreement ValueInst mapInst
        hlayout hgeometry hlayers hcomplete hends hempty
    ...
    | @expand right newLeft hash hhas hleft hright =>
      ...
      have hleftAgreement := hbinaryAgreement.zero_left hgeometry hhas
      obtain ⟨hleftShape, _, _, _⟩ := ProgressiveTree.updated_layer_contents_of_skipped ValueInst mapInst updates
        hlayout hleftClones
        hleftAgreement hnext hstart hstop hbinary hnonempty hzeroShape hleft
      ...
      obtain ⟨hrightShape, hrightEnds, hrightContents⟩ :=
        rightCorrect .ProgressiveZero right (.zero factor next.val) hrightEnds
          (hclone.zero_right hgeometry hhas) (hbinaryAgreement.zero_right hgeometry hhas)
          (hlayers.zero_right hgeometry hhas)
      ...
      · have hread := ProgressiveTree.get_after_expanded_layer_of_skipped ValueInst mapInst updates
          hlayout hleftClones
          hleftAgreement (hash := hash) (right := right) hnext hstart hstop hbinary
      ...
      | node _ hleftShape hrightShape =>
        ...
        obtain ⟨hnewRightShape, hnewRightEnds, hrightContents⟩ :=
          rightCorrect right newRight hrightShape' hrightEnds
            (hclone.node_right hgeometry hhasSuccess) (hbinaryAgreement.node_right hgeometry hhasSuccess)
            (hlayers.node_right hgeometry hhasSuccess)
        ...
        have hnewLeftShape : newLeft.Shape factor (2 * depth.val) := by
          ...
          · exact hleftShape
          · have hnonempty := (ProgressiveTree.has_updates_in_range_true ValueInst mapInst hhas).1
            exact (ProgressiveTree.updated_layer_contents_of_skipped ValueInst mapInst updates hlayout
              (hclone.node_left hgeometry hhas) (hbinaryAgreement.node_left hgeometry hhas)
        ...
        · rcases hleft with ⟨hempty, rfl⟩ | ⟨hhas, hleft⟩
          · cases pending with
            | none => exact node_get_same_left ValueInst hnext hstop hroute
            | some value =>
              rw [node_get_same_left ValueInst hnext hstop hroute]
              exact hlayers.node_here hgeometry hempty query value (by omega) (by scalar_tac) hhi hget
          · exact ProgressiveTree.get_after_updated_layer_override_of_skipped ValueInst mapInst updates hlayout
              (hclone.node_left hgeometry hhas) (hbinaryAgreement.node_left hgeometry hhas)
...
/-- Recursive bulk contents permit matching pending entries both in layers
skipped by false range answers and in suffixes skipped by the maximum guard.
Matching values may also be skipped inside selected binary subtrees. No
range-value exclusion or reflection law is needed. The numeric extent and
extension completeness justify skipped zero regions. -/
theorem ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_skipped_ranges {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    ...
    (hclone : before.BulkRetainedCloneOn (fun value => ValueInst.corecloneCloneInst.clone value = ok value)
      ValueInst mapInst updates factor maximum depth)
    (hbinary : before.BulkBinarySkippedValuesAgree ValueInst mapInst updates factor maximum depth)
    {oldLength : Nat} {newLength : Std.Usize}
    (hextent : newLength.val ≤ maximum.elim oldLength (fun last => max (last.val + 1) oldLength))
    (hlayers : before.BulkLayerSkippedValuesAgree ValueInst mapInst updates maximum newLength.val depth)
  ...
  exact bulk_contents_aux ValueInst mapInst updates hlayout hextent hcomplete
    (Std.U32.max - depth.val) depth before after (Nat.le_refl _) hshape hends hclone hbinary hlayers hskipped hupdate

/-- Public bulk contents with separate input laws for skipped progressive
layers, skipped suffixes, and skipped binary ranges. These conditions use
actual external answers and original reads; successful read-back is derived
from the rebuilding result without range-value laws. -/
theorem ProgressiveTree.with_updated_leaves_shape_contents_of_skipped_ranges {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    ...
    (hbinary : ∀ maximum, mapInst.max_index updates = ok maximum →
  ...
  | ok maximum =>
    ...
    exact ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_skipped_ranges ValueInst mapInst updates
      hlayout (hclone maximum hmax) (hbinary maximum hmax) (hextent maximum hmax)
      (hlayers maximum hmax) (hskipped maximum hmax) hcomplete hshape hends hupdate

/-- Recursive bulk contents permit matching pending entries both in layers
skipped by false range answers and in suffixes skipped by the maximum guard.
Only selected binary queries still need pending-value exclusion. The numeric
extent and extension completeness justify skipped zero regions. -/
theorem ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_layer_agreement {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    {maximum : Option Std.Usize} {before after : ProgressiveTree T} {depth : Std.U32}
    (hclone : before.BulkRetainedCloneOn (fun value => ValueInst.corecloneCloneInst.clone value = ok value)
      ValueInst mapInst updates factor maximum depth)
    (hrange : before.BulkBinaryRangeOn (update_map.RangeExcludesValuesAt mapInst updates)
      ValueInst mapInst updates factor maximum depth)
    {oldLength : Nat} {newLength : Std.Usize}
    (hextent : newLength.val ≤ maximum.elim oldLength (fun last => max (last.val + 1) oldLength))
    (hlayers : before.BulkLayerSkippedValuesAgree ValueInst mapInst updates maximum newLength.val depth)
    ...
    after.Shape factor depth.val ∧ after.EndsAfter factor depth.val newLength.val ∧
      ProgressiveTree.BulkContents ValueInst mapInst updates factor before after depth newLength := by
  exact ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_skipped_ranges
    ValueInst mapInst updates hlayout hclone
    (ProgressiveTree.BulkBinarySkippedValuesAgree.of_ranges hrange)
    hextent hlayers hskipped hcomplete hshape hends hupdate

/-- Public bulk contents with separate input laws for skipped progressive
layers, skipped suffixes, and selected binary queries. These laws refer only
to actual external answers and input reads; successful read-back is derived from the rebuilding result. -/
theorem ProgressiveTree.with_updated_leaves_shape_contents_of_layer_agreement {T U : Type}
    (ValueInst : Value T) (mapInst : update_map.UpdateMap U T) (updates : U)
    {factor : Option Std.Usize} {packingDepth : Std.Usize}
    (hlayout : tree.PackingLayout ValueInst factor packingDepth)
    {before after : ProgressiveTree T}
    ...
    (hrange : ∀ maximum, mapInst.max_index updates = ok maximum →
      before.BulkBinaryRangeOn (update_map.RangeExcludesValuesAt mapInst updates)
        ValueInst mapInst updates factor maximum 0#u32)
    {oldLength : Nat} {newLength : Std.Usize}
    (hextent : ∀ maximum, mapInst.max_index updates = ok maximum →
      newLength.val ≤ maximum.elim oldLength (fun last => max (last.val + 1) oldLength))
    ...
    after.Shape factor 0 ∧ after.EndsAfter factor 0 newLength.val ∧
      ProgressiveTree.BulkContents ValueInst mapInst updates factor before after 0#u32 newLength := by
  exact ProgressiveTree.with_updated_leaves_shape_contents_of_skipped_ranges
    ValueInst mapInst updates hlayout hclone
    (fun maximum hmax => ProgressiveTree.BulkBinarySkippedValuesAgree.of_ranges (hrange maximum hmax))
    hextent hlayers hskipped hcomplete hshape hends hupdate
...
theorem ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_skipped {T U : Type}
    ...
    after.Shape factor depth.val ∧ after.EndsAfter factor depth.val newLength.val ∧
      ProgressiveTree.BulkContents ValueInst mapInst updates factor before after depth newLength := by
  exact ProgressiveTree.with_updated_leaves_recursive_shape_contents_of_layer_agreement
    ValueInst mapInst updates hlayout hclone hrange.binary_layers hextent
    (ProgressiveTree.BulkLayerSkippedValuesAgree.of_ranges newLength.val
      (hrange.layers (fun _ _ h => h))) hskipped hcomplete hshape hends hupdate
...
theorem ProgressiveTree.with_updated_leaves_shape_contents_of_skipped {T U : Type}
    ...
    after.Shape factor 0 ∧ after.EndsAfter factor 0 newLength.val ∧
      ProgressiveTree.BulkContents ValueInst mapInst updates factor before after 0#u32 newLength := by
  exact ProgressiveTree.with_updated_leaves_shape_contents_of_layer_agreement
    ValueInst mapInst updates hlayout hclone
    (fun maximum hmax => (hrange maximum hmax).binary_layers) hextent
    (fun maximum hmax => ProgressiveTree.BulkLayerSkippedValuesAgree.of_ranges newLength.val
      ((hrange maximum hmax).layers (fun _ _ h => h))) hskipped hcomplete hshape hends hupdate
