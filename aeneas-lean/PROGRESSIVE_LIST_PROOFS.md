...
| `pop_front` | Drop the specified prefix, reindex remaining values; reject oversized drops unchanged | `PopFront/Total.lean`: every in-bounds removal succeeds, represents exactly `contents.drop n`, and preserves `BackingValid`. Nonzero removal clears pending updates and requires successful identity cloning only for retained values, occupied capacity only for the retained suffix, and empty-default-map laws. Packing layout and these clone/capacity/map premises are conditional on nonzero removal; zero remains an unconditional no-op. `PopFront/BuilderTotal.lean` establishes actual streaming reconstruction success and the complete builder invariant, requiring only terminating clones for success. `BuilderLength.lean` derives the exact recorded count without clone laws. `PopFront/Success.lean` proves public success, retained count, rebuilt backing validity, and installation of the actual default map without clone identity or default-map semantics; the full sequence theorem reuses it. `PopFront/Length.lean` and `Capacity.lean` prove occupied retained-layer capacity necessary and sufficient for nonzero in-bounds success under terminating retained clones and default construction, with no unused-successor bound. `PopFront/State.lean` proves oversized removals return the exact bounds error unchanged and every returned Rust error restores the original list. `Contents.lean` retains successful-execution content and backing proofs with packing, clone, and default-map laws conditional on nonzero removal. No intermediate vector, assumed iterator output, successful-subcall premise, or opaque milhouse-method model. `PopFront/Caches.lean` proves nonzero success rebuilds entirely cleared caches without an old-cache invariant; every returned state preserves any zero-accepting cache predicate, including errors and zero removal. |
...
- `Tree/ProgressiveList/PopFront/BuilderLength.lean` derives the exact streamed
  count from actual successful execution, with no clone or builder-validity
  law. `BuilderTotal.lean` now requires only terminating clones for execution
  success; its sequence contract still requires identity for retained values.
  `PopFront/Success.lean` lifts the weaker execution law to actual nonzero
  removal, proving the retained backing count, rebuilt backing validity, and
  installation of the actual default map without default-map semantics.
  The existing total sequence theorem reuses that execution proof.
  `PopFront/Length.lean` obtains the retained count from any successful nonzero
  removal without clone identity or a final-capacity premise. `Capacity.lean`
  then proves occupied retained-layer capacity necessary and sufficient for
  nonzero in-bounds success, under terminating retained clones and default
  construction. No unused-successor capacity is required. Successful contents
  and backing preservation now require packing, clone, and default-map laws
  only for nonzero removal, matching the total contract's unconditional zero
  no-op.
...
- `Tree/PackedLeaf/Insert.lean` proves necessary and sufficient position and
  ...
  from identity only at that slot. `PackedLeaf/BulkUpdate.lean` uses these facts
...
Latest front-removal capacity checkpoint (through `90acf16`): all focused
