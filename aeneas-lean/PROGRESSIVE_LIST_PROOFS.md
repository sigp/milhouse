...
- Selected-layer `Layer.lean`, progressive/list `Contents.lean`, and list
  ...
  Positive numeric binary selection remains upfront on rebuilding; its necessity
  is still open. The old sufficient contracts retain their signatures as adapters.
...
- Binary `Density.lean` now preserves density using numeric effects of reached
  ...
  support the materialization criteria above; binary numeric selection necessity
  remains open.
...
- `Tree/BulkUpdate/Skipped.lean` permits a pending value in a range
  ...
  selection remains an independent condition.
...
Latest apply-updates skipped-binary materialization checkpoint:
