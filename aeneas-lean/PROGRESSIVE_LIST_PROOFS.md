...
- `ProgressiveTree/Rebase/CacheInputs.lean` defines `RebaseBaseCachesOn`,
  requiring base binary-cache validity only in matching progressive layers
  entered after pointer checks. It omits absent and shared suffixes and has
  adapters from the previous full-base invariants. Progressive cache
  preservation, the finite-collision bridge, and all eleven public rebase
  cache/validity contracts use this weaker premise (`1547078`, foundation
  `e512df3`). Validity within each selected binary layer is still required;
  this is not yet an exact account of every imported binary subtree.
...
Latest selected base-cache checkpoint (`1547078`, foundation `e512df3`):
