...
| `rebase`, `rebase_on` | Preserve values, length, and pending updates while changing sharing only | `Rebase/SelectedCaches.lean` gives both public cache equivalences under selected content soundness and actual success, without layout, geometry, density, capacity, accurate-length, query-success, map, or representation assumptions. The selected cache scope follows actual optional metadata, packing results, and machine clamps; existing cache contracts use it through dense-input adapters. `Rebase/SelectedContents.lean` proves exact materialized backing-content preservation from actual success and the selected `RebaseContentInputs` law, without layout, shape, density, capacity, accurate-length, or query-success assumptions. `Contents.lean` gives merged-sequence variants using that law, retaining geometry/layout only for traversal and the existing map-clone read/extent laws for nonmutating rebasing. Existing content contracts are adapters to these results. `Rebase/Total.lean`: both operations terminate successfully and preserve the represented merged sequence and `BackingValid`, under input representation/backing validity, an accurately sized dense base, packing layout, soundness of true element `eq` at corresponding reached leaves and packed element agreement only when every paired `ne` returns false, terminating comparisons only on paths selected by the pointer/hash/length guards and through the first true packed element `ne`, and equal-length nonzero cache-shortcut soundness. `Rebase/Ready.lean` proves both public success criteria with no separate packing-layout, global query-success, whole-tree shape, or representable-layer premise. Success is equivalent to an immediate missing/shared input stop or the actual packing-query results together with the selected arithmetic, binary geometry, and element-call requirements; nonmutating rebase additionally requires the actual pending-map clone on every branch. Progressive `Ready.lean` recovers packing queries from actual entered-node execution, and `PackingQueries.lean` records factor and defaulted-depth results without positivity or power-of-two/coherence laws. Packed-factor logarithm outcomes remain explicit query conditions; no blanket metadata totality is claimed. `Tree/Rebase/GeometrySuccess.lean` proves the binary selected geometry and comparison conditions jointly necessary and sufficient on arbitrary inputs; pointer, zero-input, and hash shortcuts omit unreached checks. Progressive `Requirements.lean` retains both machine clamps, and `SelectedSuccess.lean` derives every metadata calculation and recursive call from the returned queries and selected bounds. The existing progressive/public success lemmas and total contracts use the complete criterion through invariant adapters. `Rebase/Conditions.lean` retains the earlier comparison-only specialization under global invariants; packing and geometry for content/cache correctness and model fidelity retain separate audit obligations. In-place rebase preserves exact pending-map state and requires no map or element-clone law; nonmutating rebase requires only the actual pending-map clone to terminate, preserve lookup results after the original backing fallback, and match logical extent, without raw read or maximum identity, and returns exactly that cloned map. `Rebase/Backing.lean` proves backing preservation without equality, hash, or clone laws. `Rebase/State.lean` proves in-place error restoration and unchanged metadata/observers; nonmutating observer preservation uses necessary-and-sufficient laws on the actual maximum and emptiness query results. Different maxima may describe the same length; failure, divergence, and checked overflow are preserved without requiring exact maximum identity or successful queries. `Rebase/Caches.lean` adds successful-execution and total specifications preserving arbitrary cache predicates indexed by the stored values and binary/progressive depth. The actual recursion preserves original progressive caches and may import binary caches from the base; base progressive-cache validity is unnecessary. `ProgressiveTree/Rebase/CacheInputs.lean` restricts base binary-cache validity to matching progressive layers entered after pointer checks, omitting absent and shared suffixes. All eleven public cache/validity contracts and the finite-collision bridges use this scope, with adapters from full-base validity. `Tree/Rebase/KindReflection.lean` further derives the actual binary action category from input decisions at accurate dense metadata. `Tree/Rebase/CacheInputs.lean` uses it to omit all base-cache laws for no-ops, require full base validity for whole-base replacement, and recurse into selected child caches for rebuilt nodes; the progressive scope uses this binary law within reached layers. `Rebase/OriginalCaches.lean` at both levels limits original validity to retained caches, while `Rebase/HashInputs.lean` separately limits original comparison validity to reached nonzero hash shortcuts. Eight successful/total public cache contracts use these scopes; the error-inclusive wrapper retains full original validity and cleared-input contracts derive both scopes. The progressive step certificate retains child calls even when final pointer checks reuse the original node. `Rebase/CacheEquivalence.lean` proves the retained-original and imported-base validity laws jointly necessary and sufficient for each successful public result, under the existing content laws and geometry. The binary/progressive reflection proofs recover these laws from actual output validity; the public equivalences need no cache-validity, represented-sequence, or pending-map clone/read/maximum premise. Backing-cache results require no pending-map clone semantics. `Rebase/Validity.lean` derives the operational cache-shortcut law from reference-valid input caches and collision soundness on finite corresponding binary input pairs, omitting original zero-cache nodes and all progressive caches. It provides successful-execution and total public specifications and validity on every returned in-place state. `Rebase/Cleared.lean` proves both total rebase variants from cleared original caches without any collision assumption. `Tree/Rebase/Soundness.lean` and `ProgressiveTree/Rebase/Soundness.lean` restrict equality laws to the actual pair of backing trees, omit element laws on pointer shortcuts and unequal-length packed vectors, and are used by every public rebase contents/cache specification. Pointer sharing now prunes all descendant comparison, equality, and collision obligations; eligible nonzero equal-hash/equal-length shortcuts also prune descendant comparison, equality, and collision laws. `Rebase/Pointer.lean` proves in-place rebasing onto shared backing returns the complete original list and nonmutating rebasing performs exactly the pending-map clone, without structural, packing, or semantic comparison assumptions. `Tree/Rebase/ComparisonInputs.lean` tracks supplied optional lengths and exact child splits; the binary/progressive success inductions and every public total contract use those metadata-specific scopes. `ElementComparisons.lean` proves both sufficiency and reflection for the external short-circuit loop, leaving pairs after the first true `ne` unconstrained. `Tree/Rebase/PackedSoundness.lean` further restricts packed element soundness to vectors whose paired `ne` calls all return false, and proves this guarded law necessary and sufficient for sound positive vector equality. Any other paired result removes all soundness obligations for that vector, including earlier false answers; every public rebase contents/cache contract uses the weaker law. Reference hash computation and shared cache writes are deferred with TreeHash; cache-shortcut soundness remains an explicit assumption |
...
- `Tree/UpdateMap/CowWriteBack.lean`,
  ...
  occupied-handle, lookup agreement after the backing fallback, and maximum
  agreement laws compose with actual `get_cow` and `into_mut` to prove complete in-bounds sequence
...
Latest selected-cache checkpoint (`e88ce19`, foundations `2307f24`,
