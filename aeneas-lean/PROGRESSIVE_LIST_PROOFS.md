...
| `Decode` methods | Decode SSZ contents, including empty/invalid/zero-sized-element cases | Actual fixed/variable streaming bodies and metadata now extract with the real error enum and local offset-reader/default-formatting models (UPSTREAM_BUGS issue 19). Differential release tests preserve the previous parser's error order, partial-builder finalization, and exact errors. Sequence reconstruction, malformed-input specifications, and roundtrip proofs remain pending |
