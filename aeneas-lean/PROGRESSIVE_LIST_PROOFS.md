...
- `Tree/UpdateMap/CowWriteBack.lean`,
  `Tree/ProgressiveList/CopyOnWrite/FallbackConsuming.lean`: entry-location,
  occupied-handle, lookup agreement after the backing fallback, and maximum
  agreement laws at the selected fallback compose with actual `get_cow` and
  `into_mut` to prove complete in-bounds sequence replacement. Theorem
  `get_cow_into_mut_spec_of_fallback` derives the handle and every consuming subcall,
...
Latest CoW fallback checkpoint (`c7310ef`, exact criteria `5ad99dc`): eleven
