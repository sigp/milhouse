...
- `Tree/BulkUpdate/Skipped.lean` permits a pending value in a range
  ...
  existence criteria: backing-density preservation still uses it, and the
  weaker content conditions still need connecting to those criteria. Separate
  execution contracts now use the guards above without range correctness.
...
Latest apply-updates execution-guard checkpoint:
