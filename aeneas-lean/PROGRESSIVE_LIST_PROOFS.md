...
- Binary `Density.lean` now preserves density using numeric effects of reached
  ...
  lemmas using standard Lean axioms. The weaker content conditions still need
  lifting into the public valid-materialization existence criteria, and binary
  numeric selection necessity remains open.
- `Tree/BulkUpdate/Guards.lean` describes the actual missing-update guards,
  ...
  rebuilding branch. The no-op has no rebuilding conditions. These contracts
  do not yet weaken the public valid-materialization criteria's binary range
  assumptions. This checkpoint adds 19 public lemmas using standard Lean axioms.
- `Tree/BulkUpdate/Skipped.lean` permits a pending value in a range
  ...
  necessity to dense, exact list materialization. These results do not yet
  weaken the binary reflection premise of the public valid-materialization
  existence criteria: the weaker content conditions still need connecting to
  those criteria. Backing preservation now uses the numeric/skipped-agreement
  contracts above, and separate execution contracts use the guards without
  range correctness.
...
Latest apply-updates binary density range checkpoint:
