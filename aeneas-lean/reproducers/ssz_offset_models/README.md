...
This suite compares the local offset-decoding model with freshly extracted
`ethereum_ssz` 0.10.0 code. Run from the repository root:
...
`14210df0e27ccd7d9e6a05b8085cbd438e4bbc65`). It checks formatting, runs four
native tests, validates dependency/source provenance, generates fresh Lean,
and compiles both generated modules and the three comparisons. Each proof
...
64-bit targets. Native encoding checks supplement the decoder comparisons;
they do not constitute an encoding source proof or a release-profile proof.
...
All five existing source-comparison suites also pass with the shared runner
extension.

The suite adds two direct comparisons and one composition check. Encoder
state transitions, other SSZ/model boundaries, borrowed CoW, and the remaining
assumption review stay open. The main library proofs and model-root inventory
